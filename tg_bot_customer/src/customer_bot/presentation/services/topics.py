...
from customer_bot.infrastructure.http import BackendClient, TelegramTopicDTO
    ...
    async def _cache_topic(self, telegram_id: int, topic: TelegramTopicDTO) -> None:
        if topic.message_thread_id is None:
            await self._redis.set(
                customer_redis_keys.topic_thread_by_kind(telegram_id, topic.topic_kind),
                "",
            )
            return
        await self._redis.set(
            customer_redis_keys.topic_kind_by_thread(
                telegram_id,
